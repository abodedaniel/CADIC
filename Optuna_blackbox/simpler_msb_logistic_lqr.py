...
class logistic_agent():
    def __init__(self, n_subn, num_subband, Pmax, init_subband):
        self.t = 0
        self.K = n_subn
        self.num_subband = num_subband
        self.Pmax = Pmax
        self.init_subband = init_subband

    def int_measure(self,sum_int, length):
        if len(sum_int.shape) == 2:
            sum_int = np.expand_dims(sum_int,-1)
        sum_int_ = sum_int[:,:,-length:]
        cum_sumk = np.mean(sum_int_,axis=-1)
        return cum_sumk

    def switch(self, cum_sumk, LQR, LQR_S0, LQR_S1):
        channel_arg = np.argsort(cum_sumk)[:,0:self.num_subband]
        subn_channel = np.zeros((self.K,self.num_subband))
        ...
        subn_channel[to_switch_to_two_channels,0] = self.init_subband[to_switch_to_two_channels] #the first channel is init
        ...
        to_switch_to_init_channel = np.invert(to_switch_for_improvement)
        subn_channel[to_switch_to_init_channel,:] = np.expand_dims(self.init_subband[to_switch_to_init_channel],-1)

        ## To update init subband, comment out to keep fixed
        self.init_subband = subn_channel[:,0]
        return subn_channel
    ...
    def act(self, state, params, sumint):
        ...
        cum_sumk = self.int_measure(sumint, length)
        
        Ch_alloc = self.switch(cum_sumk, LQR_int, LQR_S0, LQR_S1)
        return Ch_alloc