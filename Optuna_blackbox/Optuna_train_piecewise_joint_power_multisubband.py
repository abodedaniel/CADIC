...
def objective(trial):
    ...
    X0 = trial.suggest_int('X0', 0, 50, step=2)
    ...
    for ep in range(n_eps):  # Run episodes
        ...
        for t in range(n_steps):  # Run one episode
            pow_action, ch_action = agent.joint_act(state,[K,X0,S2,S3],sum_int, env.init_ch) #np.ones(env.n_subnetworks) # 
...
np.savez('1episodemeanmaxMixedplantsJointpower2switch50100150Random'+str(n_eps)+str(n_steps)+str(n_trials)+'subband_trials' + str(n_subnetworks) + sampler_ +'.npz',best_trials = study.best_trials, all_trials=study.trials, study=study)
