...
class SubnetworkEnv(gym.Env):
    ...
    def __init__(self, n_subnetworks=25, n_subband=3, reward_type='sum', num_step = 100):
        ...
        self.source_env = env_subnetwork(numCell= self.n_subnetworks, numSubbands=self.n_subbands, problem='power', steps=self.num_step+1, numerology=5, RB_per_Subband = 3)
    ...
    def reset(self, seed=None, options='SISA'):
        ...
        #print(reset_action)
        rxPow = np.mean(self.source_env.ul_rxPow[0:self.n_subnetworks,self.n_subnetworks:,:,:,0],axis=3)
        if options == 'SISA':
        ...
        power_action = self.action_space.sample()
        _ = self.source_env.traffic_aware_joint_power_channel_step(power_action, chl_action=np.tile(subband_action,(self.n_subbands,1)).T,time_index=self.time_index)
    ...
    def step(self, power_action, subband_action):
        self.time_index = self.time_index + 1
        #valid_plants = np.arange(num_of_subnetworks)[env.activity_indicator[self.time_index].astype(bool)]
        if len(subband_action.shape) == 1:
            subband_action = np.tile(subband_action,(self.n_subbands,1)).T
        self.source_env.subn_plant_control._gen_sensor_data()
        _ = self.source_env.traffic_aware_joint_power_channel_step(power_action, chl_action=subband_action,time_index=self.time_index)
        ...
        if self.time_index == self.num_step:
            truncated = True
        self.info["rate"].append(self.source_env.ul_avRate)
        self.info["action"].append(subband_action)
