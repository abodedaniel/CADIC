...
from simpler_msb_logistic_lqr import logistic_agent
...
start = time.time()
def objective(trial):
    LQR_S0 = trial.suggest_int('LQR_S0', 0, 400, step=5)
    LQR_S1 = trial.suggest_int('LQR_S1', LQR_S0, 500, step=5)
    ...
    for ep in range(n_eps):  # Run episodes
        ...
        state, info, sum_int, init_subband = env.reset()
        agent = logistic_agent(n_subn=env.n_subnetworks, num_subband= n_subband, Pmax=0, init_subband=init_subband)
        #print(sum_int)
        for t in range(n_steps):  # Run one episode
            action = agent.act(state,[LQR_S0,LQR_S1,length],sum_int) #np.ones(env.n_subnetworks) # 
...
np.savez('simpleruseintLQRmultisubband_intlength'+str(n_eps)+str(n_steps)+str(n_trials)+'subband_trials' + str(n_subnetworks) + sampler_ +'.npz',best_trials = study.best_trials, all_trials=study.trials, study=study)
