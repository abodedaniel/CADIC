...
def objective(trial):
    ...
    X0 = trial.suggest_int('X0', 0, 300, step=2)
    S2 = trial.suggest_int('S2', 0, 150, step=2)
    S3 = trial.suggest_int('S3',S2, 400, step=2)
    p = trial.suggest_float('p', 0, 1, step=0.01)
    ...
    for ep in range(n_eps):  # Run episodes
        ...
        for t in range(n_steps):  # Run one episode
            pow_action, ch_action = agent.joint_act_w_penalty(state,[K,X0,S2,S3,p],sum_int, env.init_ch) #np.ones(env.n_subnetworks) # 
...
np.savez('meanmaxpenaltyJointpower2switchRandom'+str(n_eps)+str(n_steps)+str(n_trials)+'subband_trials' + str(n_subnetworks) + sampler_ +'.npz',best_trials = study.best_trials, all_trials=study.trials, study=study)
