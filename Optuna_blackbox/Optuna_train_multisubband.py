...
from multi_subband_logistic_lqr import logistic_agent
...
def objective(trial):
    ...
    LQR_S1 = trial.suggest_int('LQR_S1', LQR_S0, 1000, step=5)
    ...
    for ep in range(n_eps):  # Run episodes
        ...
        for t in range(n_steps):  # Run one episode
            action = agent.act(state,[LQR_S0,LQR_S1],sum_int, env.init_ch) #np.ones(env.n_subnetworks) # 
...
np.savez('instantlqr_avAllsumint_multisubband'+str(n_eps)+str(n_steps)+str(n_trials)+'subband_trials' + str(n_subnetworks) + sampler_ +'.npz',best_trials = study.best_trials, all_trials=study.trials, study=study)
