...
start = time.time()
def objective(trial):
    S2 = trial.suggest_int('S2', 0, 150, step=1)
    ...
    for ep in range(n_eps):  # Run episodes
        ...
        for t in range(n_steps):  # Run one episode
            action = agent.act_2switch(state,[S2,S3],sum_int, env.init_ch) #np.ones(env.n_subnetworks) # 
...
np.savez('2switchRandomAllmultisubband'+str(n_eps)+str(n_steps)+str(n_trials)+'subband_trials' + str(n_subnetworks) + sampler_ +'.npz',best_trials = study.best_trials, all_trials=study.trials, study=study)
