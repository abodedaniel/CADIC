...
start = time.time()
def objective(trial):
    S1 = trial.suggest_int('S1', 0, 50, step=1)
    S2 = trial.suggest_int('S2', S1, 150, step=1)
    S3 = trial.suggest_int('S3',S2, 300, step=1)
    ...
    for ep in range(n_eps):  # Run episodes
        ...
        state, info, sum_int, _ = env.reset(options='SISA')
        #print(sum_int)
        for t in range(n_steps):  # Run one episode
            action = agent.act(state,[S1,S2,S3],sum_int, env.init_ch) #np.ones(env.n_subnetworks) # 
...
np.savez('Allmultisubband'+str(n_eps)+str(n_steps)+str(n_trials)+'subband_trials' + str(n_subnetworks) + sampler_ +'.npz',best_trials = study.best_trials, all_trials=study.trials, study=study)
